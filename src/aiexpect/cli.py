...
def _markdown_summary(payload, min_trust=None) -> str:
    from .results import CATEGORY_LABELS

    s = payload["summary"]
    trust = s["trust_score"]
    head = f"{trust:.0f}/100" if trust is not None else "n/a"
    gate = ""
    if min_trust is not None and trust is not None:
        gate = "  ✅ above" if trust >= min_trust else "  ❌ below"
        gate += f" the {min_trust:g} gate"
    lines = [f"### aiexpect Trust Score: **{head}**{gate}", "",
             "| Sub-score | Value |", "|---|---|"]
    for cat, v in s["subscores"].items():
        lines.append(f"| {CATEGORY_LABELS[cat]} | {f'{v:.0f}' if v is not None else '– (not tested)'} |")
    lines += ["", f"{s['passed_checks']}/{s['total_checks']} checks passed ({s['pass_rate']:.0f}%)"]
    failed = [c for c in payload.get("checks", []) if not c["passed"]]
    if failed:
        lines += ["", "<details><summary>Failed checks</summary>", ""]
        for c in failed[:20]:
            lines.append(f"- `{c['name']}` in `{c.get('test_id') or '?'}`: {c['reason'][:200]}")
        if len(failed) > 20:
            lines.append(f"- … and {len(failed) - 20} more")
        lines += ["", "</details>"]
    lines += ["", "<sub>Generated by <a href=\"https://github.com/dmsehgal/aiexpect\">aiexpect</a></sub>"]
    return "\n".join(lines)
...
def main(argv=None) -> int:
    ...
    s.add_argument("--markdown", action="store_true", help="print a Markdown summary (for PR comments)")
    ...
    if a.cmd == "summary":
        ...
        trust = s["trust_score"]
        if a.markdown:
            print(_markdown_summary(payload, a.min_trust))
            return 0 if a.min_trust is None or (trust is not None and trust >= a.min_trust) else 1
